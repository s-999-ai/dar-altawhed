[index.html.html](https://github.com/user-attachments/files/26639530/index.html.html)
<!DOCTYPE html>
...
/* QUICK ACTIONS */
.qa{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:11px;margin-bottom:18px;}
...
<div class="qac" style="border:2px solid rgba(231,76,60,.35);background:#fff;" onclick="showAP('upload',document.querySelectorAll('.ni')[6]);setTimeout(()=>{const e=document.getElementById('auto-proc-msg');if(e)e.parentElement.scrollIntoView({behavior:'smooth'});},400)">
<div class="qic">⚡</div><div class="qtt" style="color:#e74c3c;">تطبيق تلقائي</div><div class="qsb">إجراءات بضغطة واحدة</div></div>
...
<div class="cb">
<div class="alt as">✅ يدعم ملفات <strong>AbsenceOnStudentLevel</strong> المُصدَّرة من نظام نور مباشرة — كل ورقة = طالب واحد</div>
<div class="alt ai">📋 يقرأ الملف تلقائياً: <strong>اسم الطالب · رقم الهوية · الصف · الفصل · أيام الغياب بعذر وبدون عذر · تواريخ الغياب الهجرية</strong></div>
<div class="uz" id="uz" onclick="document.getElementById('fi2').click()"
ondragover="event.preventDefault();this.classList.add('over')"
ondragleave="this.classList.remove('over')"
ondrop="event.preventDefault();this.classList.remove('over');handleMultiF(event.dataTransfer.files)">
<input type="file" id="fi2" accept=".xlsx,.xls" multiple style="display:none" onchange="handleMultiF(this.files)">
<span class="uz-ic">📊</span>
<h3 style="color:var(--g1);font-size:14px;font-weight:700;margin-bottom:4px;">اسحب ملفات AbsenceOnStudentLevel هنا أو انقر للاختيار</h3>
<p style="color:#999;font-size:12px;">يدعم ملف واحد أو أكثر .xlsx — يقرأ كل الأوراق تلقائياً</p>
<div class="uzchips">
...
<!-- AUTO APPLY PROCEDURES CARD -->
<div class="card" style="border:2px solid rgba(231,76,60,.25);">
<div class="ch" style="background:linear-gradient(135deg,#922b21,#e74c3c);">
<div class="ch-l"><div class="cic">⚡</div><h2>تطبيق الإجراءات تلقائياً — بضغطة زر واحدة</h2></div>
</div>
<div class="cb">
<div class="alt aw">⚠️ سيُطبَّق الإجراء المناسب على كل طالب تلقائياً حسب لائحة الغياب 1447هـ</div>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin-bottom:16px;">
<div style="background:#fff3cd;border:1px solid #f9e79f;border-radius:10px;padding:12px;">
<div style="font-weight:700;color:#856404;font-size:13px;margin-bottom:4px;">📢 3-4 أيام</div>
<div style="font-size:12px;color:#777;">تنبيه شفوي + إشعار ولي الأمر</div></div>
<div style="background:#fde8d8;border:1px solid #f0b27a;border-radius:10px;padding:12px;">
<div style="font-weight:700;color:#7d3c00;font-size:13px;margin-bottom:4px;">⚠️ 5-9 أيام</div>
<div style="font-size:12px;color:#777;">إنذار أول خطي + إشعار ولي الأمر</div></div>
<div style="background:#ffd7cc;border:1px solid #e74c3c;border-radius:10px;padding:12px;">
<div style="font-weight:700;color:#6b1a0a;font-size:13px;margin-bottom:4px;">🔴 10-14 يوم</div>
<div style="font-size:12px;color:#777;">إنذار ثاني + تعهد ولي الأمر</div></div>
<div style="background:#f8d7da;border:1px solid #c0392b;border-radius:10px;padding:12px;">
<div style="font-weight:700;color:#721c24;font-size:13px;margin-bottom:4px;">🚨 15+ يوم</div>
<div style="font-size:12px;color:#777;">إنذار ثالث + استدعاء ولي الأمر</div></div>
</div>
<div id="auto-proc-msg"></div>
<div class="br">
<button class="btn bd" onclick="autoApplyProcedures(false)" style="padding:11px 20px;font-size:13px;">
⚡ تطبيق على من لم يُسجَّل لهم إجراء بعد</button>
<button class="btn" style="background:linear-gradient(135deg,#922b21,#c0392b);color:#fff;padding:11px 20px;font-size:13px;" onclick="autoApplyProcedures(true)">
🔄 إعادة تطبيق على جميع المستحقين</button>
</div>
</div></div>
...
let ubuf=[];
// =============================================
// قراءة ملفات AbsenceOnStudentLevel من نور
// Row 3  col 1  = المرحلة
// Row 8  col 1  = الفصل
// Row 19 col 11 = اسم الطالب
// Row 23+ = أسطر الغياب col11=النوع col15=التاريخ col16=اليوم col21=الرقم
// =============================================
function parseNoorSheet(data){
  if(!data||data.length<20)return null;
  const r=(i,c)=>(data[i]&&data[i][c]!=null)?String(data[i][c]).trim():'';
  const grade=r(3,1);
  if(!grade||grade.includes('الصف')||!grade.includes('ثانوي'))return null;
  const section=r(8,1)||'1';
  const name=r(19,11);
  if(!name||name.includes('اسم الطالب')||name.length<3)return null;
  let excused=0,unexcused=0;
  const absences=[];
  for(let i=23;i<data.length;i++){
    const row=data[i];if(!row||row.length<22)continue;
    const seq=row[21];if(typeof seq!=='number'||seq<=0)continue;
    const absType=row[11]?String(row[11]).trim():'';
    const dateH=row[15]?String(row[15]).trim():'';
    const dayName=row[16]?String(row[16]).trim():'';
    if(!absType)continue;
    if(absType.includes('بدون عذر'))unexcused++;
    else if(absType.includes('بعذر'))excused++;
    absences.push({seq:Math.round(seq),date:dateH,day:dayName,type:absType});
  }
  let gradeNorm='أول ثانوي';
  if(grade.includes('الثاني'))gradeNorm='ثاني ثانوي';
  else if(grade.includes('الثالث'))gradeNorm='ثالث ثانوي';
  return {name,grade:gradeNorm,section,excused,unexcused,total:excused+unexcused,absences};
}

function readWorkbook(buffer){
  const XL=window.XLSX;
  const wb=XL.read(buffer,{type:'binary'});
  const results=[];
  wb.SheetNames.forEach(sname=>{
    const ws=wb.Sheets[sname];
    const data=XL.utils.sheet_to_json(ws,{header:1,defval:null});
    const student=parseNoorSheet(data);
    if(student)results.push(student);
  });
  return results;
}

function handleMultiF(files){
  if(!files||!files.length)return;
  const XL=window.XLSX;
  if(!XL){document.getElementById('ups').innerHTML='<div class="alt ad">❌ مكتبة Excel لم تُحمَّل بعد، انتظر ثانية وحاول مجدداً</div>';return;}
  document.getElementById('ups').innerHTML='<div class="alt ai">⏳ جاري قراءة '+files.length+' ملف...</div>';
  ubuf=[];
  let processed=0;
  Array.from(files).forEach(file=>{
    ...
    reader.onload=function(e){
      try{
        const students=readWorkbook(e.target.result);
        ubuf=[...ubuf,...students];
      }catch(err){console.error('Error:',file.name,err);}
      processed++;
      if(processed===files.length)showUploadPreview();
    };
    reader.readAsBinaryString(file);
  });
}

function handleF(file){if(file)handleMultiF([file]);}

function showUploadPreview(){
  if(!ubuf.length){
    document.getElementById('ups').innerHTML='<div class="alt ad">❌ لم يُعثر على بيانات — تأكد أن الملف نوع AbsenceOnStudentLevel من نظام نور</div>';
    return;
  }
  // Remove duplicate names (keep last)
  const seen={};
  ubuf=ubuf.filter(s=>{if(seen[s.name])return false;seen[s.name]=true;return true;});
  const fcount=document.getElementById('fi2').files?document.getElementById('fi2').files.length:1;
  document.getElementById('ups').innerHTML=
    `<div class="alt as">✅ تم قراءة <strong>${ubuf.length}</strong> طالب من <strong>${fcount}</strong> ملف — الغياب الجديد سيُضاف تراكمياً للقديم</div>`;
  document.getElementById('pvh').innerHTML=
    '<tr><th>#</th><th>الاسم</th><th>المرحلة</th><th>ف</th><th>بعذر ق</th><th>+ج</th><th>=</th><th>بدون عذر ق</th><th>+ج</th><th>=</th><th>الحالة</th></tr>';
  document.getElementById('pvb').innerHTML=ubuf.slice(0,25).map((r,i)=>{
    const ex=D.find(x=>x.name===r.name);
    const oE=ex?ex.excused:0,oU=ex?ex.unexcused:0;
    const tE=oE+r.excused,tU=oU+r.unexcused;
    const st=gSt(tU);
    return `<tr style="${!ex?'background:#fffbee;':''}">
      <td>${i+1}</td>
      <td class="sname">${r.name}${!ex?'<span class="badge b-bl" style="font-size:9px;margin-right:3px;">جديد</span>':''}</td>
      <td style="font-size:11px;">${r.grade}</td><td>${r.section}</td>
      <td style="color:#27ae60;opacity:.7;">${oE}</td>
      <td style="color:#27ae60;font-weight:700;">+${r.excused}</td>
      <td style="color:#27ae60;font-weight:900;">${tE}</td>
      <td style="color:#e74c3c;opacity:.7;">${oU}</td>
      <td style="color:#e74c3c;font-weight:700;">+${r.unexcused}</td>
      <td style="color:#e74c3c;font-weight:900;font-size:13px;">${tU}</td>
      <td><span class="badge ${st.bg}">${st.lb}</span></td>
    </tr>`;
  }).join('')+(ubuf.length>25?`<tr><td colspan="11" style="text-align:center;color:#aaa;padding:7px;">... و ${ubuf.length-25} طالب آخر</td></tr>`:'');
...
const pf=document.getElementById('uprog-f');
document.getElementById('uprog').style.display='block';
let up=0,ad=0;

ubuf.forEach((r,idx)=>{
  // Match by name first (since نور files don't have SID), fallback to sid
  let s=D.find(x=>x.name===r.name)||(r.sid?D.find(x=>x.sid===r.sid):null);
  if(s){
    // ACCUMULATE: add new to existing
    s.excused=s.excused+r.excused;
    s.unexcused=s.unexcused+r.unexcused;
    s.total=s.excused+s.unexcused;
    // Update grade/section if they were missing
    if(!s.grade||s.grade==='أول ثانوي')s.grade=r.grade;
    if(!s.section||s.section==='1')s.section=r.section;
    // Append new absence details (avoid duplicates by date+type)
    if(r.absences&&r.absences.length){
      if(!s.absences)s.absences=[];
      const existingKeys=new Set(s.absences.map(a=>a.date+'|'+a.type));
      const newAbs=r.absences.filter(a=>!existingKeys.has(a.date+'|'+a.type));
      // Re-sequence
      const startSeq=s.absences.length+1;
      newAbs.forEach((a,i)=>{a.seq=startSeq+i;});
      s.absences=[...s.absences,...newAbs];
    }
    up++;
  }else{
    // New student
    const nid=Math.max(...D.map(s=>s.id),0)+1;
    D.push({
      id:nid,name:r.name,sid:r.sid||'',
      grade:r.grade,section:r.section,
      excused:r.excused,unexcused:r.unexcused,total:r.total,
      absences:r.absences||[],notices:[],excuseRequests:[],
      exempted:false,adminNotes:[],records:[],
      procedures:{step:0,history:[],legalStatus:null}
    });
    ad++;
  }
  pf.style.width=Math.round(((idx+1)/ubuf.length)*100)+'%';
});
...
document.getElementById('ups').innerHTML=
  `<div class="alt as">✅ اكتمل التحديث: <strong>${up}</strong> طالب تراكم غيابه + <strong>${ad}</strong> طالب جديد — الملاحظات والإنذارات محفوظة ✔️</div>`;
btn.disabled=false;btn.innerHTML='✅ تأكيد — جمع الغياب';
updD();renderT();updNB();
}
...
// =============================================
//  AUTO APPLY PROCEDURES - بضغطة واحدة
// =============================================
function autoApplyProcedures(force){
  const today=nH();
  let counts={verbal:0,first:0,second:0,third:0,skipped:0};
  
  D.forEach(s=>{
    const u=s.unexcused;
    if(u<3){skipped++;return;}
    
    if(!s.notices)s.notices=[];
    
    // Determine what level they need
    let targetNotice=null;
    let targetLabel='';
    
    if(u>=15&&u<20){
      // إنذار ثالث
      const has=s.notices.find(n=>n.type.includes('إنذار ثالث'));
      if(!has||force){
        targetNotice={type:'إنذار ثالث + استدعاء ولي الأمر',notes:'تم التطبيق التلقائي — '+u+' أيام غياب بدون عذر',date:today};
        counts.third++;
      }else counts.skipped++;
    }else if(u>=10&&u<15){
      // إنذار ثاني
      const has=s.notices.find(n=>n.type.includes('إنذار ثاني'));
      if(!has||force){
        targetNotice={type:'إنذار ثاني + تعهد ولي الأمر + تحويل موجه',notes:'تم التطبيق التلقائي — '+u+' أيام غياب بدون عذر',date:today};
        counts.second++;
      }else counts.skipped++;
    }else if(u>=5&&u<10){
      // إنذار أول
      const has=s.notices.find(n=>n.type.includes('إنذار أول'));
      if(!has||force){
        targetNotice={type:'إنذار أول خطي + إشعار ولي الأمر',notes:'تم التطبيق التلقائي — '+u+' أيام غياب بدون عذر',date:today};
        counts.first++;
      }else counts.skipped++;
    }else if(u>=3&&u<5){
      // تنبيه شفوي
      const has=s.notices.find(n=>n.type.includes('تنبيه'));
      if(!has||force){
        targetNotice={type:'تنبيه شفوي + إشعار ولي الأمر',notes:'تم التطبيق التلقائي — '+u+' أيام غياب بدون عذر',date:today};
        counts.verbal++;
      }else counts.skipped++;
    }
    ...
    if(targetNotice){
      if(force){
        // Remove old notice of same level before adding
        s.notices=s.notices.filter(n=>!n.type.includes(targetNotice.type.split('+')[0].trim()));
      }
      s.notices.push(targetNotice);
    }
  });
  
  sv();renderT();updNB();renderAl();
  
  const total=counts.verbal+counts.first+counts.second+counts.third;
  const msg=`<div class="alt as" style="margin-top:10px;">
    ✅ تم تطبيق الإجراءات على <strong>${total}</strong> طالب:<br>
    📢 تنبيه شفوي: <strong>${counts.verbal}</strong> طالب |
    ⚠️ إنذار أول: <strong>${counts.first}</strong> طالب |
    🔴 إنذار ثاني: <strong>${counts.second}</strong> طالب |
    🚨 إنذار ثالث: <strong>${counts.third}</strong> طالب
    ${counts.skipped?`<br><span style="color:#888;font-size:12px;">• ${counts.skipped} طالب تم تخطيه (مسجَّل مسبقاً أو أقل من 3 أيام)</span>`:''}
  </div>`;
  document.getElementById('auto-proc-msg').innerHTML=msg;
}
